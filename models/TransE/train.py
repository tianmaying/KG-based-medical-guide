...
# Load data
with open("../../data/OpenKG/dsc.nlp-bigdatalab.org.ttl", "r", encoding='utf8') as inf:
...
for line in lines:
    s = line[0]
    if s.find('<') != -1:
        s = line[0][line[0].rfind('/') + 1:line[0].find('>')]
    p = line[1]
    if p.find('<') != -1:
        p = p[p.rfind('/') + 1:p.find('>')]
    if p.find('#') != -1:
        p = 'R_' + p[p.find('#') + 1:]
    line[2] = line[2][:len(line[2]) - 2]
    o = line[2]
    if o.find('<') != -1:
        o = line[2][line[2].rfind('/') + 1:line[2].find('>')]
    #if p == '疾病相关症状':
    triples.append([s, p, o])

triples_valid = triples
